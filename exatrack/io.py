...
TrackSegmentSequence    : Keras Sequence wrapping segment_tracks for model.fit()
...
Dependencies: config.py, numpy, pandas, numba
"""
...
def segment_tracks(track_list, LocErr_list, dt_list, batch_size,
    ...
    Connects to the Numba-accelerated _segment_tracks_core for efficiency.
...
class TrackSegmentSequence(tf.keras.utils.Sequence):
    """
    Keras Sequence that pre-segments tracks into fixed-length batches.

    Wraps segment_tracks for use with model.fit(). Each __getitem__ call
    returns one batch of (tracks, LocErrs, dts, masks, isfirsts).
    """
    ...
    def __init__(self, track_list, LocErr_list, dt_list,
        ...
        self.tracks, self.LocErrs, self.dts, self.masks, self.isfirsts = \
            segment_tracks(track_list, LocErr_list, dt_list, batch_size,
    ...
    def on_epoch_end(self):
        if self.shuffle:
            (self.tracks, self.LocErrs, self.dts,
             self.masks, self.isfirsts) = segment_tracks(
                self.track_list, self.LocErr_list, self.dt_list,
                self.batch_size, self.segment_length,
                self.min_segment_length, self.cutoff_batch_treshhold,
                self.shuffle)
